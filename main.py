...
class StickerMaker:
    ...
    def start_snipping(self):
        ...
        self.snip_window.attributes("-fullscreen", True)
        # ensure it stays on top (macOS fix)
        self.snip_window.attributes("-topmost", True)

        # Set transparency (Mac & Windows Fix)
        try:
            self.snip_window.attributes("-alpha", 0.3)  # Set to 30% opacity
        except tk.TclError:
            print("Opacity might not work on macOS, using alternative transparency.")

        # create a canvas widget to trace on and change the cursor to crosshair (macOS fix)
        self.canvas = tk.Canvas(self.snip_window, cursor="crosshair", bg="gray")
        # make sure the canvas fills the entire window
        self.canvas.pack(fill=tk.BOTH, expand=True)
    ...
    def capture_snip(self, event):
        ...
        # macOS Fix: Ensure screen recording permissions are granted
        try:
            img = pyautogui.screenshot()
        except Exception as e:
            print("Error: Screen recording permissions needed on macOS.")
            return
        ...
        # if there are more than 2 points, fill the polygon with white
        if len(self.points) > 2:
            cv2.fillPoly(mask, [np.array(self.points, dtype=np.float32)], 255)  # macOS fix
        ...
        # show the main window again (macOS fix: use iconify if withdraw fails)
        self.root.deiconify()
